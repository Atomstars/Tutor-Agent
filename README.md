...
If you are using PowerShell, SQL statements like `CREATE DATABASE ...` must be executed **inside PostgreSQL tools** (`psql` or pgAdmin Query Tool), not directly in PowerShell.

### 1) Create DB using `psql`
```powershell
psql -U postgres -h localhost -p 5432 -d postgres
```
Then run in the `psql` prompt:
```sql
CREATE DATABASE tutoragent;
```
(If you already use user `postgres`, you can skip creating another user.)

### 2) Set backend env vars in PowerShell
```powershell
cd backend
$env:DB_URL="jdbc:postgresql://localhost:5432/tutoragent"
$env:DB_USER="postgres"
$env:DB_PASSWORD="postgres"
$env:JWT_SECRET="change-this-secret-key-change-this-secret-key-1234"
...
### 3) Run frontend in a second PowerShell terminal
```powershell
cd frontend
